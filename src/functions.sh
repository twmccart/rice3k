...
function clean_and_split_vcf() {
	...
	for chromosome in chr{01,02,03,04,05,06,07,08,09,10,11,12}; do (
		bcftools view --exclude-uncalled --exclude-types 'indels' -r ${chromosome} -O z -o ${cultivar}.${chromosome}.cleaned.vcf.gz ${cultivar}.full.vcf.gz
		tabix ${cultivar}.${chromosome}.cleaned.vcf.gz) &
	done
	wait
}
